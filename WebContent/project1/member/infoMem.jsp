...
<body class="w3-light-grey w3-content" style="max-width:2400px">
	...
	<div class="w3-center" style="margin-left:35%;margin-top:10%;">
		<div style = "margin-top: 50px; margin:auto; max-width: 1450px; min-width:700px;">
   			<table>
   				<tr><th>회원 구분</th><th>아이디</th><th>이름</th><th>닉네임</th><th>성별</th><th>전화번호</th><th>이메일</th>
   				<th>수정</th><th>탈퇴</th></tr>
   			<c:forEach var="m" items="${mem }">
   				<tr><td>${m.mclass==1?"일반":"관계자" }</td><td>${m.id }</td><td>${m.name }</td><td>${m.nick }</td><td>${m.gender==1?"남":"여" }</td>
   					<td>${m.tel }</td><td>${m.email }</td><td><a href="updateForm.me?id=${m.id }&mclass=${m.mclass }">수정</a></td>
   				<c:if test="${m.id != 'admin' }">
					<td><a href="javascript:del('${m.id }')">[강제탈퇴]</a></td>
				</c:if>
				</tr>
   			</c:forEach>
			</table>
