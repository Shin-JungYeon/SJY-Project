...
<body class="w3-light-grey w3-content" style="max-width:2400px">
	...
	<%-- 일반 회원 --%>
		<c:if test="${mclass == 1 }">
		<a href="myboardinfo.do?id=${sessionScope.login }" onclick="w3_close()" class="w3-bar-item w3-button">작성글 관리</a> 
	...
	<div class="w3-center" style="margin-left:300px;margin-top:50px;">
		<div style="margin-top: 50px; margin:auto; max-width: 1450px; min-width:700px;">
			<table>
				<tr><th>No.</th><th>적립 포인트</th><th>사용 포인트</th><th>적립 내용</th><th>사용 내용</th><th>누적 포인트</th></tr>
			<c:forEach var="mpl" items="${myplist }"></c:forEach>
				<tr><td>${mpl.seq }</td>
					<td><c:if test="${mpl.kbn==1 }">${mpl.point }</c:if></td>
					<td><c:if test="${mpl.kbn==2 }">${mpl.point }</c:if></td>
					<td><c:if test="${mpl.kbn==1 }">${mpl.content }</c:if></td>
					<td><c:if test="${mpl.kbn==2 }">${mpl.content }</c:if></td>
					<td></td></tr>
			</table>		
		</div>
	</div>
