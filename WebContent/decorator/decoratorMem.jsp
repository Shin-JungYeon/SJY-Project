...
<body class="w3-light-grey w3-content" style="max-width:2400px">
	<nav class="w3-sidebar w3-bar-block w3-white w3-text-grey w3-collapse w3-top w3-center" style="z-index:3;width:300px;font-weight:bold" id="mySidebar"><br>
		...
		<a href="info.do" onclick="w3_close()" class="w3-bar-item w3-button">정보 상세보기</a>
		<a href="info.do" onclick="w3_close()" class="w3-bar-item w3-button">정보 상세보기</a>  
	
	<%-- 관리자 --%>
	<c:if test="${classify == 0 }">
		<a href="infoGen.do" onclick="w3_close()" class="w3-bar-item w3-button">일반 회원 목록</a> 
		<a href="infoCon.do" onclick="w3_close()" class="w3-bar-item w3-button">공연관계자 회원 목록</a>
		<a href="infoReq.do" onclick="w3_close()" class="w3-bar-item w3-button">회원가입 요청 내역</a>
		<a href="infoPoReq.do" onclick="w3_close()" class="w3-bar-item w3-button">포인트 신청 내역</a>
	</c:if>
	
	<%-- 일반 회원 --%>
		<c:if test="${classify == 1 }">
		<a href="infoGen.do" onclick="w3_close()" class="w3-bar-item w3-button">작성글 목록 확인</a> 
		<a href="infoCon.do" onclick="w3_close()" class="w3-bar-item w3-button">포인트 내역 확인</a>
	</c:if>
	
	<%-- 관계자 회원 --%>
	<c:if test="${classify == 2 }">
		<a href="infoGen.do" onclick="w3_close()" class="w3-bar-item w3-button">작성글 목록 확인</a> 
		<a href="infoCon.do" onclick="w3_close()" class="w3-bar-item w3-button">이력 확인</a>
	</c:if>	
	</nav>
